...
def main():
    ...
    parser.add_argument("-o", "--output", type=str, default="./proof_of_concept/", help="Path to the output folder.")
    ...
    for feature_name, feature_map in feature_map_dict["thumbnail"].items():
        thumbnail_feature_map_mean, thumbnail_feature_map_std = calc_mean_std(feature_map) # dim = C
        thumbnail_feature_maps_precomputed[f"{feature_name}_mean"] = thumbnail_feature_map_mean
        thumbnail_feature_maps_precomputed[f"{feature_name}_std"] = thumbnail_feature_map_std

    other_featuer_maps_precomputed = defaultdict(dict)
    for img_name, feature_maps in feature_map_dict.items():
        if img_name == "thumbnail": continue
        ...
        for feature_name, feature_map in feature_maps.items():
            ...
            other_featuer_maps_precomputed[img_name][f"{feature_name}_mean"] = other_feature_map_mean
            other_featuer_maps_precomputed[img_name][f"{feature_name}_std"] = other_feature_map_std
    ...
    for img_name, featuer_maps_precomputed in other_featuer_maps_precomputed.items():
        ...
        for feature_name, feature_map_precomputed in featuer_maps_precomputed.items():
            cos_sim_value = cos_sim(thumbnail_feature_maps_precomputed[feature_name], feature_map_precomputed)
            cos_sim_dict[feature_name].append(cos_sim_value)
    print("Drawing ...")
    for key1, key2 in zip(list(cos_sim_dict.keys())[0::2], list(cos_sim_dict.keys())[1::2]):
    ...
    cos_sim_dict = defaultdict(list)
    for i, (img_name_a, featuer_maps_precomputed_a) in enumerate(other_featuer_maps_precomputed.items()):
        for j, (img_name_b, featuer_maps_precomputed_b) in enumerate(other_featuer_maps_precomputed.items()):
            if i < j:
                ...
                for feature_name in featuer_maps_precomputed_a.keys():
                    cos_sim_value = cos_sim(featuer_maps_precomputed_a[feature_name], featuer_maps_precomputed_b[feature_name])
    ...
    for key1, key2 in zip(list(cos_sim_dict.keys())[0::2], list(cos_sim_dict.keys())[1::2]):
        ...
        sns.boxplot(ax=ax1, x="distance", y="cos sim", data=pd.DataFrame({"distance":list(map(lambda x: round(x, 2), value1[:,0])), "cos sim":value1[:,1]}))
        ...
        sns.boxplot(ax=ax2, x="distance", y="cos sim", data=pd.DataFrame({"distance":list(map(lambda x: round(x, 2), value2[:,0])), "cos sim":value2[:,1]}))
