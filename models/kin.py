...
class KernelizedInstanceNorm(nn.Module):
    ...
    def update_cache_table(self, instance_means, instance_stds, y_anchors, x_anchors):
        instance_means = instance_means.squeeze(-1).squeeze(-1)
        instance_stds = instance_stds.squeeze(-1).squeeze(-1)
        for instance_mean, instance_std, y_anchor, x_anchor in zip(
            instance_means, instance_stds, y_anchors, x_anchors,
        ):
            self.mean_table[y_anchor, x_anchor, :] = instance_mean
            self.std_table[y_anchor, x_anchor, :] = instance_std
    ...
    def forward_normal(self, x, eps=1e-9):
        x_mean, x_std = self.calc_mean_std(x)
        x = (x - x_mean) / (x_std + eps) #* self.weight + self.bias
        return x
