    ...
    def __init__(self, input_nc, ndf=64, size=None, opt=None):
        ...
        if "patch" in self.opt.netD:
            ...
            self.final_linear = nn.Sequential(
                EqualLinear(
                    channels[4] * size * size, channels[4], activation="fused_lrelu"
                ),
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style):
        ...
        weight = weight.view(
            b * self.out_channel, in_channel, self.kernel_size, self.kernel_size
        )
        ...
        if self.upsample:
            ...
            weight = weight.transpose(1, 2).reshape(
                b * in_channel, self.out_channel, self.kernel_size, self.kernel_size
            )
