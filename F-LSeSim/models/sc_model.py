...
class SCModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument(
            "--T", type=float, default=0.07, help="temperature for similarity"
        )
        parser.add_argument(
            "--lambda_spatial",
            type=float,
        ...
        parser.add_argument(
            "--lambda_style", type=float, default=0.0, help="weight for style loss"
        )
        parser.add_argument(
            "--lambda_identity", type=float, default=0.0, help="use identity mapping"
        )
        parser.add_argument(
            "--lambda_gradient",
            type=float,
            default=0.0,
            help="weight for the gradient penalty",
        )
    ...
    def __init__(self, opt, norm_cfg=None):
        ...
        if self.isTrain:
            ...
            self.criterionSpatial = losses.SpatialCorrelativeLoss(
                opt.loss_mode,
                opt.patch_nums,
                opt.patch_size,
                opt.use_norm,
                ...
                self.optimizer_F = torch.optim.Adam(
                    [
                        ...
                        },
                        {
                            "params": list(
                                filter(
                                    lambda p: p.requires_grad, self.netF.parameters()
                                )
                            )
                        },
                    ],
    ...
    def forward(self):
        ...
        if (
            self.opt.lambda_identity + self.opt.lambda_spatial_idt > 0
        ) and self.opt.isTrain:
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        if self.opt.lambda_gradient > 0.0:
            self.loss_D_Gradient, _ = losses.cal_gradient_penalty(
                netD, real, fake, real.device, lambda_gp=self.opt.lambda_gradient
            )  #
